...
// ProxiPixel local patch: copy `source`'s watermark box back into `target`.
// Used to reset a shared scratch buffer between relocation candidates instead
// of cloning the whole image each time (see buildSmallAnchorRelocationCandidate).
function restoreImageDataBox(target, source, position) {
    const { x, y, width, height } = position;
    const stride = target.width * 4;
    for (let row = 0; row < height; row++) {
        const start = (y + row) * stride + x * 4;
        target.data.set(source.data.subarray(start, start + width * 4), start);
    }
}
...
function buildSmallAnchorRelocationCandidate({
    ...
    // ProxiPixel local patch: reuse a shared scratch buffer instead of
    // `cloneImageData(originalImageData)` per candidate. The original full clone
    // is O(image), and on uniform backgrounds thousands of candidates survive
    // the pre-check, so detection took tens of seconds. removeWatermark only
    // writes the watermark box and the scorers read the box plus the
    // (unmodified) surrounding halo, so restoring just the box afterwards leaves
    // the scratch identical to the original for the next candidate. The winning
    // candidate's imageData is rebuilt once by the caller. Output-identical.
    const candidateImageData = scratchImageData;
    ...
    // Compute residual visibility before restoring (it reads the scratch box);
    // skip it when the thresholds already failed, matching the original order.
    const residualVisibility = thresholdFail
        ? null
        : assessWatermarkResidualVisibility({
            imageData: candidateImageData,
            position,
    ...
    restoreImageDataBox(candidateImageData, originalImageData, position);

    if (thresholdFail) return null;
    if (residualVisibility.visible) return null;

    return {
        imageData: null, // rebuilt by the caller for the winning candidate only
        alphaMap,
        position,
...
function refineSmallFixedLocalAnchorGeometry({
    ...
    let best = null;
    // ProxiPixel local patch: one scratch buffer reused across all candidates
    // (see buildSmallAnchorRelocationCandidate). Each candidate restores its box,
    // so the scratch stays equal to the original between candidates.
    const scratchImageData = cloneImageData(originalImageData);
    ...
    // ProxiPixel local patch: rebuild the winning candidate's image once (the
    // search reused a shared scratch and never kept per-candidate pixels).
    if (best) {
        const winnerImageData = cloneImageData(originalImageData);
        removeWatermark(winnerImageData, best.alphaMap, best.position, { alphaGain: best.alphaGain });
        best.imageData = winnerImageData;
    }
